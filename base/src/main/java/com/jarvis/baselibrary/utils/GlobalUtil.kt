...
object GlobalUtil {
    ...
    val appPackage: String
        get() = AppHelper.currentApplication.packageName
     ...
     */
    val appName: String
        get() = AppHelper.currentApplication.resources.getString(AppHelper.currentApplication.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = AppHelper.currentApplication.packageManager.getPackageInfo(
            appPackage,
            0
        ).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            AppHelper.currentApplication.packageManager.getPackageInfo(
                appPackage,
                0
            ).longVersionCode
        } else {
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(
                        AppHelper.currentApplication.contentResolver,
                        Settings.Secure.ANDROID_ID
                    )
                } catch (e: Exception) {
                    LogUtil.w("get android_id with error$e")
                }
            ...
            var uuid: String = SpUtil.get("uuid", "")
    ...
    fun getString(resId: Int): String {
        return AppHelper.currentApplication.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return AppHelper.currentApplication.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return AppHelper.currentApplication.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = AppHelper.currentApplication.packageManager.getApplicationInfo(
                appPackage,
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            AppHelper.currentApplication.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = AppHelper.currentApplication.packageManager
